---
name: nim-mcp-tools
description: 'Use for Nim symbol navigation and project diagnostics. MANDATORY: Prefer specialized MCP tools (nimFindSymbols, nimFindReferences, nimListSymbols, nimCheckProject) over grep, ripgrep, or shell commands to save tokens and ensure semantic precision.'
---

# Nim MCP Tools

## Core Mandate
AI agents MUST prefer specialized MCP tools over general-purpose instruments (grep, shell commands, `read_file` with manual parsing) for all Nim symbol-related and Nim project-diagnostics tasks. This is critical for:
...
When the user asks to "check" a Nim project, interpret that as a diagnostics request by default. Do **not** implicitly run `nimble test`, project test suites, or other potentially long-running validation commands after collecting diagnostics unless the user explicitly asks for tests or broader verification.
...
Treat diagnostics whose paths are under a project's `nimbledeps/` directory as **dependency diagnostics**, not project-local diagnostics. By default, exclude those diagnostics from the main report unless the user explicitly asks to include dependency issues or all diagnostics.

Do **not** pair `nimFindSymbols` or `nimCheckProject` with `grep`, `ripgrep`, or shell search "just to double-check". If the task is about a Nim symbol or Nim project diagnostics, MCP tools own the search unless they have already failed.
...
### 4. Check the Current Nim Project for Diagnostics
Do NOT approximate project diagnostics by grepping build logs or manually scanning files.
1. Call `nimCheckProject()`.
2. Treat the result as the answer unless the user explicitly asked for additional validation such as tests or builds.
3. Filter out diagnostics whose paths are under `nimbledeps/` by default, because those are dependency diagnostics rather than project-local diagnostics.
4. Use the remaining diagnostics to report errors, warnings, and hints for the current Nim project context.
5. If the user asked for only a subset (for example, only errors or only warnings), filter the returned diagnostics by severity before presenting them.
...
3. Do not present the fallback as equivalent in precision to `nimFindSymbols`, `nimFindReferences`, `nimListSymbols`, or `nimCheckProject`.
...
- **NO IMPLICIT TEST RUNS**: Do not run `nimble test` or other test commands as part of a "check project" request unless the user explicitly asked for tests.
- **HIDE DEPENDENCY NOISE BY DEFAULT**: Do not present diagnostics from `nimbledeps/` as project diagnostics unless the user explicitly asks to include dependency diagnostics.
...
- Running `nimble test` automatically after `nimCheckProject()` for a generic "check this project" request.
- Reporting `nimbledeps/` diagnostics as if they were project-local issues without the user asking for dependency diagnostics.
