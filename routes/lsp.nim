...
): Future[seq[CompletionItem]] {.async: (raises: [CancelledError, OSError, IOError, RegexError, NimsuggestError]).} =
...
): Future[seq[Location]] {.async: (raises: [CancelledError, OSError, IOError, RegexError, AsyncProcessError, NimsuggestError]).} =
  with (params.position, params.textDocument):
    asyncSpawn ls.addProjectFileToPendingRequest(id.uint, uri)
    ...
    withValue(ls.openFiles, uri, value):
      if config.useNimTrack.get(false) and not value.changed:
        let ch = ls.getCharacter(uri, line, character)
        if ch.isNone:
          return @[]
        let projectFile = await value.projectFile
...
proc declaration*(
    ls: LanguageServer, params: TextDocumentPositionParams, id: int
): Future[seq[Location]] {.async: (raises: [CancelledError, OSError, IOError, RegexError, NimsuggestError]).} =
  with (params.position, params.textDocument):
    asyncSpawn ls.addProjectFileToPendingRequest(id.uint, uri)
    let ns = await ls.tryGetNimsuggest(uri)
    if ns.isNone:
...
): Future[ExpandResult] {.async: (raises: [CancelledError, OSError, IOError, RegexError, NimsuggestError]).} =
  with (params.position, params.textDocument):
    let ns = await ls.tryGetNimsuggest(uri)
    if ns.isNone:
...
proc extensionSuggest*(
...
): Future[SuggestResult] {.async: (raises: [CancelledError, KeyError, OSError, RegexError]).} =
  ...
  if projectFile != "" and projectFile notin ls.projectFiles:
    ...
    withValue(ls.openFiles, uri, value):
      projectFile = await value.projectFile
...
): Future[seq[SymbolInformation]] {.async: (raises: [CancelledError, OSError, IOError, RegexError, NimsuggestError]).} =
...
): Future[Option[Hover]] {.async: (raises: [CancelledError, OSError, IOError, RegexError, AsyncProcessError, AsyncStreamError, NimsuggestError]).} =
  with (params.position, params.textDocument):
    let config = ls.getWorkspaceConfiguration()
    asyncSpawn ls.addProjectFileToPendingRequest(id.uint, uri)
    let nimsuggest = await ls.tryGetNimsuggest(uri)
...
): Future[JsonNode] {.async: (raises: [CancelledError, OSError, IOError, RegexError, NimsuggestError]).} =
  with (params.position, params.textDocument):
    asyncSpawn ls.addProjectFileToPendingRequest(id.uint, uri)
    let nimsuggest = await ls.tryGetNimsuggest(uri)
    if nimsuggest.isNone:
...
): Future[WorkspaceEdit] {.async: (raises: [CancelledError, KeyError, OSError, IOError, RegexError, AsyncProcessError, NimsuggestError]).} =
...
): Future[seq[InlayHint]] {.async: (raises: [CancelledError, OSError, IOError, RegexError, NimsuggestError]).} =
...
): Future[Option[SignatureHelp]] {.async: (raises: [CancelledError, OSError, IOError, RegexError, NimsuggestError]).} =
...
): Future[Option[TextEdit]] {.async: (raises: [CancelledError, OSError, IOError, AsyncProcessError, AsyncStreamError]).} =
...
): Future[seq[TextEdit]] {.async: (raises: [CancelledError, OSError, IOError, AsyncProcessError, AsyncStreamError]).} =
...
proc documentHighlight*(
    ls: LanguageServer, params: TextDocumentPositionParams, id: int
): Future[seq[DocumentHighlight]] {.async: (raises: [CancelledError, OSError, IOError, RegexError, NimsuggestError]).} =
  with (params.position, params.textDocument):
    asyncSpawn ls.addProjectFileToPendingRequest(id.uint, uri)
    let nimsuggest = await ls.tryGetNimsuggest(uri)
    if nimsuggest.isNone:
...
proc shutdown*(ls: LanguageServer, input: JsonNode): Future[JsonNode] {.async: (raises: []).} =
  debug "Shutting down"
...
): Future[AsyncProcessRef] {.async: (raises: [CancelledError, OSError, AsyncProcessError]).} =
  await startProcess(
    "nimble",
    arguments = args,
    options = {UsePath},
...
proc tasks*(ls: LanguageServer, conf: JsonNode): Future[seq[NimbleTask]] {.async: (raises: [CancelledError, OSError, AsyncProcessError, AsyncStreamError]).} =
...
): Future[RunTaskResult] {.async: (raises: [CancelledError, ValueError, OSError, AsyncProcessError, AsyncStreamError]).} =
...
): Future[ListTestsResult] {.async: (raises: [CancelledError, ValueError, OSError, IOError, AsyncProcessError, AsyncStreamError]).} =
  let config = ls.getWorkspaceConfiguration()
...
): Future[RunTestProjectResult] {.async: (raises: [CancelledError, ValueError, OSError, IOError, AsyncProcessError, AsyncStreamError]).} =
  let config = ls.getWorkspaceConfiguration()
  let workspaceRoot = ls.lspInitializeParams.getRootPath
  let nimPath = await ls.getNimPath(config, workspaceRoot)
  if nimPath.isNone:
...
#Notifications
proc initialized*(ls: LanguageServer, _: JsonNode): Future[void] {.async: (raises: []).} =
  debug "Client initialized."
  maybeRegisterCapabilityDidChangeConfiguration(ls)
  await maybeRequestConfigurationFromClient(ls)

proc cancelRequest*(ls: LanguageServer, params: CancelParams): Future[void] {.async: (raises: []).} =
  if params.id.isSome:
    let id = params.id.get.getInt.uint
    withValue(ls.pendingRequests, id, value):
      if value.request != nil:
        debug "Cancelling: ", id = id
        await value.request.cancelAndWait()
        value.state = prsCancelled
        value.endTime = now()
  ...
  with params:
    ...
    withValue(ls.openFiles, uri, value):
      ...
      for line in contentChanges[0].text.splitLines:
        value.fingerTable.add line.createUTFMapping()
        file.writeLine line
      ls.scheduleFileCheck(uri)
