...
proc execSpawn(
  ...
  if slot.isActive:
    debug "execSpawn: slot already active, skipping",
  ...
  debug "execSpawn: spawning nimsuggest", projectFile = projectFile
  try:
...
proc execCheckKnown(
  ...
  let nsOpt = slot.resolvedNs
  if nsOpt.isNone:
    debug "execCheckKnown: slot not ready, skipping", uri = uri
    return
  ...
  debug "execCheckKnown: result",
    uri = uri, isKnown = isKnown, intendedProjectFile = intendedProjectFile
  ...
  of RoutingDecision.ACCEPT:
    discard # File is known — nothing to do.
