...
proc addQueryToQueue*( # Used to be called `queryAt`
  ...
  if fileInfo == nil:
    debug "routeQuery: no fileInfo — returning empty", uri = uri, kind = $q.kind
    q.responseFuture.complete(@[])
    return q.responseFuture

  # Unified slot healing: covers nil slots and stale (non-owning) slots.
  # Previously nil slots were given up on immediately while stale slots were
  # healed — an inconsistency, since both cases can be resolved the same way
  # by scanning the pool.
  if fileInfo.slot == nil or not fileInfo.slot.ownsUri(uri):
    if fileInfo.slot == nil:
      debug "routeQuery: fileInfo.slot is nil — scanning pool",
        uri = uri, kind = $q.kind
    else:
      debug "routeQuery: fileInfo.slot does not own uri — scanning pool",
        uri = uri, kind = $q.kind, slotProject = fileInfo.slot.projectFile
    let current = ls.pool.slotForUri(uri)
    if current.isNone:
      debug "routeQuery: no slot owns uri in pool — returning empty",
        uri = uri, kind = $q.kind
      q.responseFuture.complete(@[])
      return q.responseFuture
    debug "routeQuery: slot healed",
      uri = uri, kind = $q.kind, newSlot = current.get.projectFile
    fileInfo.slot = current.get

  # Guard against enqueuing to a terminal slot. SPAWNING and READY are fine
  # (processQueries waits for SPAWNING → READY before dispatching). CRASHED
  # may recover via auto-restart, so we allow it through. STOPPED and
  # PERMANENT_FAILURE will never recover — return empty immediately.
  if fileInfo.slot.state in {SlotState.STOPPED, SlotState.PERMANENT_FAILURE}:
    debug "routeQuery: slot is in terminal state — returning empty",
      uri = uri, kind = $q.kind, state = $fileInfo.slot.state
    q.responseFuture.complete(@[])
    return q.responseFuture

  debug "routeQuery: enqueuing to slot",
    uri = uri, kind = $q.kind, slotProject = fileInfo.slot.projectFile,
