...
type RegExp* {.importjs.} = ref object ## Regular Expressions for JavaScript target.
  ...
  lastIndex* {.importjs.}: cint
    ## Index at which to start the next match (read/write property).
...
): RegExp {.importjs: "(new RegExp(@))".} ## Creates a new RegExp object.
